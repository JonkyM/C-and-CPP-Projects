...
int BST::GetHeight()
{
  return (HeightHelper(root)-1); //1 is subtracted from the returned value so it matches the real height.
}

int BST::HeightHelper(TNode* curnode) //Calculates the height of each subtree and returns the largest between the right and left one 
{                                     //The returned value will be one number larger than the real height so we account for that later.
  int LeftHeight = 0;
  int RightHeight = 0;
  if(curnode == NULL){
    return 0;
  }
  LeftHeight = 1 + HeightHelper(curnode->left);
  RightHeight = 1 + HeightHelper(curnode->right);
...
double BST::GetIHRatio()
{
  ...
  double RealH = GetHeight();
  double IdealH = floor(log2(length));
  return (RealH/IdealH);
}
