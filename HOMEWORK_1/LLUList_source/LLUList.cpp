...
double LLUList::getMean() {
  ...
  double result;
  while (nodeIter != NULL) {
    runTotal += nodeIter->item;
    nodeIter = nodeIter->next;
  }
  result = (runTotal/LLUList::GetLength());
  return result;
}

double LLUList::getStdev() {
  double mean = LLUList::getMean();
  double meanDiffSum = 0;
  double result; 
  LNode* nodeIter = ListStart;
  while (nodeIter != NULL) {
    meanDiffSum += pow((nodeIter->item - mean),2.0);
    nodeIter = nodeIter->next;
  }
  result = sqrt(meanDiffSum/LLUList::GetLength());
  return result;
}
...
LLUList LLUList::DuplicateNV(int N) {
  ...
  for (int loc=0; loc<N; loc++) {
    curPos = ListStart;
    for (int pos = ((N-1)-loc); pos>0; pos--) { //nested for loop used to add the items in backwards order so PutItem places them in the correct order
      LLUList::GetNextItem();
    }
    dupeList.PutItem(LLUList::GetNextItem());
  }
  return dupeList;
}
