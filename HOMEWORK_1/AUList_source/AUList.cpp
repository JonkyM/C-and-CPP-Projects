...
double AUList::getMean() {
  double listTotal = 0; //define both as doubles so integer division does not happen
  double arthMean = 0;
  for (int loc=0; loc<length; loc++) {
    listTotal += ListItems[loc];
  }
  arthMean = (listTotal/length);
  return arthMean;
}

double AUList::getStdev() {
  double mean = AUList::getMean();
  double meanDiffSum = 0;
  double result;
  for (int loc=0; loc<length; loc++) {
    meanDiffSum += pow((ListItems[loc] - mean),2.0);
  }
  result = sqrt(meanDiffSum/length);
  return result;
}
...
AUList AUList::DuplicateNV(int N) {
  ...
  for (int loc=0; loc<N; loc++) {
    dupeList.PutItem(ListItems[loc]);
  }
  return dupeList;
}
